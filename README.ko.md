...
| "statusline 배치 바꿔줘" · "제목을 하늘색으로 해줘" | `/media:statusline` | statusline 허브 — 항목 켜고 끄기, 숫자 패턴 배치, 부분별 스타일까지 한 곳에서 |
...
- **업데이트는 사용 중인 탭만** — Claude Code 세션을 여러 개 열어 두면
  세그먼트는 실제로 사용 중인 탭에서만 업데이트됩니다(타이핑·스크롤·탭
  전환 전부 사용으로 칩니다). 다른 탭은 마지막 줄을 얼린 채 유지하고 —
  진행 바와 시간만 멈춥니다 — 돌아오는 순간 따라잡습니다. 원래 쓰던
  statusline은 모든 세션에서 계속 살아 움직입니다 — 게이트는 플러그인
  줄에만 걸립니다. 모든 세션에서 업데이트하려면
...
- **꾸미기는 `/media:statusline` 하나로** — 항목을 켜고 끄고, 레이아웃을
  ...
  `━━──`부터 `smooth` 부분 블록, `knob` 슬라이더 노브, 재생 중 흘러가는
  `wave`·`pulse`·`eq`·`notes` 파형까지 14가지 프리셋, 또는 아무 두 글자),
  볼륨 아이콘과 바
  모양(`block`/`progress`/`stairs`), 출력 장치 아이콘 — 그리고 `off`로
  어떤 부분이든 숨길 수 있습니다.
- 긴 제목은 marquee 방식으로 흘러갑니다. 볼륨 항목은 아이콘 + 볼륨량 높이
  바 + 퍼센트(`🔉 ▄ 45%`)로, 출력 장치 아이콘은 장치 종류에 맞춰(`🎧`
  Bluetooth, `📺` HDMI, `📶` AirPlay, `🔊` 스피커) 표시됩니다. 색은 표준
  16색 SGR만 씁니다 — 일반 텍스트로 되돌리려면
  `/media:config statusline.color off`(또는 `NO_COLOR`)를 쓰세요.
- 켜고 끄는 빠른 토글과 **statusline 초기화**는 `/media:config`에
  있습니다. 키마다 `reset`으로 개별 복원도 됩니다.
...
`/media:statusline`의 Items 탭에서 "Output device"를 체크하거나, 숫자
패턴으로 원하는 위치에 배치하면 됩니다.
