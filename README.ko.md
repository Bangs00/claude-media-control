...
- **배치는 `/media:statusline`** — 프리셋을 미리보기로 보면서 고르거나,
  `123/456` 같은 숫자 패턴 하나로 정합니다. 숫자가 항목(곡 정보, 앱, 볼륨,
  진행 바, 시간, 출력 장치)이고, `/`가 줄바꿈, 숫자 순서가 곧 표시
  순서입니다.
- **스타일은 `/media:style`** — 부분마다 굵게/기울임/색, 재생·일시정지
  강조색, 진행 바 문자(기본 `line` `━━──`; `blocks` `██░░`, `wave` `~~--`,
  `dots` `●●○○`, 또는 아무 두 글자), 볼륨 아이콘까지 바꿉니다. 키마다
  `reset`으로 개별 복원되고, 한 번에 전부 되돌릴 수도 있습니다.
