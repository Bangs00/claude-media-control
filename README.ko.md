...
| — | `/media:config` | 표시 기능 켜고 끄기 (진행 바, statusline, 스펙트럼) |
...
보여줄지(곡 정보, 진행 바, 시간, 스펙트럼), 한 줄로 붙일지 여러 줄로 나눌지는
`/media:statusline`에서 고르면 됩니다. 세그먼트에는 기본으로 ANSI 스타일이
입혀져 나옵니다 — 재생 상태에 따라 색이 바뀌는 아이콘과 진행 바, 굵은 제목,
기울임꼴 아티스트, 색을 입힌 스펙트럼(단색, 또는 `spectrum.style`로 켜는
위치 기반 무지개색). 일반 텍스트로 되돌리려면
