...
| "statusline 배치 바꿔줘" | `/media:statusline` | 미리보기로 레이아웃을 고르거나, 숫자 패턴으로 줄과 순서까지 직접 구성 |
| "제목을 하늘색으로 해줘" | `/media:style` | statusline 항목별 스타일 — 부분마다 굵게/기울임/색, 진행 바 문자, 볼륨 아이콘 |
| "statusline 설정해줘" | `/media:config` | 인터랙티브 설정 — 배치와 모든 표시 기능 켜고 끄기 (진행 바, 히스토리, 색상, marquee) |
| — | `/media:doctor` | 빌드 / 권한 / fallback 상태 진단 |

원한다면 statusline에 지금 재생 중인 곡을 띄울 수도 있습니다 —
[docs/statusline.ko.md](docs/statusline.ko.md)를 참고하세요.
`/media:statusline`은 배치 예시를 **미리보기로 직접 보여주면서** 레이아웃을
고르게 해 주고, 어떤 항목을(곡 정보, 앱, 볼륨, 진행 바, 시간, 출력 장치)
어떤 순서로, 몇 번째 줄에 올릴지까지 정할 수 있습니다. 프리셋(Standard /
Stacked)이 마음에 안 들면 `Custom…`에서 `123/456` 같은 숫자 패턴 하나를
채팅 입력창에 바로 치면 됩니다 — 숫자가 항목, `/`가 줄바꿈, 숫자 순서가 곧
표시 순서입니다. 항목은 저장한 순서 그대로 그려지기 때문에 "시간을 맨 앞에",
"출력 장치를 앞으로" 같은 요청도 그대로 반영됩니다. 볼륨 항목은 아이콘 +
볼륨량 높이 바 + 퍼센트(`🔉 ▄ 45%`)로, 출력 장치 아이콘은 장치 종류에 맞춰
(`🎧` Bluetooth, `📺` HDMI, `📶` AirPlay, `🔊` 스피커) 표시됩니다. 30칸보다
긴 제목은 marquee 방식으로 흘러가고(`statusline.marquee`), 세그먼트에는
기본으로 ANSI 스타일이 입혀져 나옵니다 — 재생 상태에 따라 색이 바뀌는
아이콘과 진행 바, 굵은 제목과 경과 시간, 기울임꼴 아티스트. 일반 텍스트로
되돌리려면 `/media:config statusline.color off`를 실행하세요(`NO_COLOR`
환경 변수도 지원합니다). 모든 부분은 `/media:style`로 하나하나 따로 꾸밀 수
있습니다 — 제목·아티스트·앱·시간·볼륨 바와 퍼센트·출력 장치의
굵게/기울임/색, 진행 바의 재생/일시정지 색과 문자(`wave` `~~~~--`, `line`,
`dots`, 또는 아무 두 글자), 볼륨 아이콘(`♪`, 숨김, 또는 레벨별 기본
아이콘)까지. 각 키는 `reset`으로 개별 복원되고, `/media:config style
reset`이면 전부 기본값으로 돌아갑니다.
...
`/media:config`에서 "출력 장치 항목"을 체크하거나, `/media:statusline`으로
원하는 위치에 배치하면 됩니다.
