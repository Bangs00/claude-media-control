...
| "statusline 설정해줘" | `/media:config` | 인터랙티브 설정 — statusline 항목과 배치, 그리고 모든 표시 기능 켜고 끄기 (진행 바, 스펙트럼, 히스토리, 색상, marquee) |
...
여러 줄로 나눌지는 `/media:config`에서 고르면 됩니다. 30칸보다 긴
...
`/media:config`로 statusline에 미니 스펙트럼을 넣을 수도 있습니다.
...
`/media:config`에서 `output` 항목을 고르면 됩니다.
