...
| — | `/media:config` | 表示機能のオン / オフ（プログレスバー、ステータスライン、スペクトラム） |
| — | `/media:doctor` | ビルド / 権限 / フォールバックの診断 |

ステータスラインに再生中の曲を出すこともできます——
[docs/statusline.ja.md](docs/statusline.ja.md) を参照してください。どの項目を
出すか（曲情報、プログレスバー、時間、スペクトラム）、1 行にまとめるか行を
分けるかは `/media:statusline` で選べます。セグメントは最初から ANSI
スタイル付きで出力されます——再生状態で色が変わるアイコンとプログレスバー、
太字の曲名、斜体のアーティスト、色付きのスペクトラム（単色、または
`spectrum.style` で有効にする位置ベースのレインボー）。プレーンテキストに
戻したいときは `/media:config statusline.color off` を実行してください
（`NO_COLOR` 環境変数も有効です）。
