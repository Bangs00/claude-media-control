...
| 「ステータスラインの並びを変えて」 | `/media:statusline` | プレビューでレイアウトを選ぶか、項目の並び順を自分で構成 |
...
くれて、どの項目を（曲情報、アプリ、プログレスバー、時間、出力デバイス）
どの順序で出すか、1 行にまとめるか行を分けるかまで決められます。プリセットが
合わなければ `Custom…` で、表示する項目と先頭に置く項目をひとつずつ選べます。
項目は保存した順序どおりに描画されるので、「時間を先頭に」「出力デバイスを
前に」といった指定もそのまま反映されます。30 セルより長いタイトルはマーキー
式にスクロールし（`statusline.marquee`）、セグメントは最初から ANSI スタイル
付きで出力されます——再生状態で色が変わるアイコンとプログレスバー、太字の
曲名と経過時間、斜体のアーティスト。プレーンテキストに戻したいときは
