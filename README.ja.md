...
- **パーツごとのスタイル**: 再生 / 一時停止のアクセントカラー、パーツ
  ...
  プログレスバー文字 31 種（デフォルトの `line`
  `━━──` から `smooth` の部分ブロック、`knob` のスライダーつまみ、バー
  全体に広がる `wave`/`pulse`/`eq`/`notes` 波形とその点字の双子、
  オーディオビジュアライザーの `spectrum`/`mirror`/`cava`/`ripple`、
  `heartbeat` の心電図 `━━┻┳━━` まで）、バーの長さ（1〜60 マス）、
