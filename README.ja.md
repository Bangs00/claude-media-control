...
| 「ステータスラインを設定して」 | `/media:config` | インタラクティブな設定——ステータスラインの項目とレイアウトに加え、すべての表示機能のオン / オフ（プログレスバー、スペクトラム、履歴、色、マーキー） |
...
1 行にまとめるか行を分けるかは `/media:config` で選べます。30 セルより
...
`/media:config` でステータスラインにミニスペクトラムを追加することも
できます。
...
`/media:config` で `output` 項目を選んでください。
