...
ステータスラインに再生中の曲を出すこともできます——設定は一度きりで、手順は
[docs/statusline.ja.md](docs/statusline.ja.md) にあります:
...
- **配置は `/media:statusline`** — プリセットをプレビューで見比べて選ぶか、
  `123/456` のような数字パターンひとつで決めます。数字が項目（曲情報、
  アプリ、音量、プログレスバー、時間、出力デバイス）、`/` が改行、数字の
  並びがそのまま表示順です。
- **スタイルは `/media:style`** — パーツごとの太字/斜体/色、再生・一時停止
  のアクセント色、バーの文字（デフォルト `line` `━━──`。`blocks` `██░░`、
  `wave` `~~--`、`dots` `●●○○`、任意の 2 文字も可）、音量アイコンまで
  変えられます。キーは `reset` で個別に戻せて、まとめて全部戻すことも
  できます。
