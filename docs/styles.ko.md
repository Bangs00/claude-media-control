...
| `━━━━━━────` | `style.progressbar.style`(문자) | `line` |
...
`style.progressbar.style`이 문자를 정합니다. statusline의 바는 10칸이고,
`/media:now` 응답의 20칸 바도 같은 문자로 그려지므로 두 곳이 항상 같은
모습입니다. 문자 선택은 색을 꺼 둔 상태에서도 적용됩니다.

![진행 바 프리셋과 볼륨 모양이 1초에 한 프레임씩 실제로 움직이는 모습](styles.gif)

### 고정 프리셋

60% 기준입니다 (`smooth`만 부분 블록이 보이도록 56%):
