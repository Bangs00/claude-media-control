...
멈춥니다. 네 가지 모두 아래 비주얼라이저처럼 **바 전체**에 걸쳐 그려지고
색으로 진행도를 나타냅니다. 길이 20과 10에서 가득 찬 모습입니다:
...
일곱 프리셋 모두 재생된 부분만이 아니라 **바 전체**에 걸쳐 그려지며, 위의
파형 네 가지도 마찬가지입니다. 색을 켜면 강조색과 흐린색의 경계가
진행도를 나타내고, 색을 끄면 재생되지 않은 꼬리 부분의 높이가 낮아져
(`notes`는 꼬리의 음표가 `·` 쉼표로 잦아들어) 모양만으로도 진행도가
읽힙니다.
