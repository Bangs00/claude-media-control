...
| `style.volume.icon` / `style.volume.style` / `style.volume.bar` / `style.volume.percent` | 音量图标 / 柱条形状 / 柱条样式 / 百分比 | `auto` / `block` / `dim` / `dim` |
...
进度条的字符由 `style.progressbar.style` 决定: `line` `━━━━━━────`（默认） ·
`blocks` `██████░░░░` · `wave` `~~~~~~----` · `dots` `●●●●●●○○○○`，或任意
两个字符表示"填充 + 空白"（`"#-"` → `######----`）。`/media:now` 回复里的
进度条也用同一组字符绘制，两处显示始终一致。音量柱条的形状由
`style.volume.style` 决定: `block`（一个随音量升降的 `▄`，默认）、
`progress`（用进度条字符画的 5 格迷你条）、`stairs`（`▂▄▆█` 阶梯）。图标
