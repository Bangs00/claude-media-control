...
同时开着多个 Claude Code 会话时，组件**只在你实际使用的那个会话里
更新**——打字、滚动、切到那个标签页，都算"在用"。其余会话保留它最后
一行、冻结不动（曲目还在，只是进度条和时间停了），你一回来，一两次刷新
就追上。你自己的状态栏在每个会话里照常实时运行——被门控的只有插件这
一行。无需任何设置。
...
<details>
<summary>门控的原理</summary>

状态栏命令在没有控制 tty 的情况下运行，所以组件沿进程祖先链找到握着本
会话终端的 Claude Code 进程，再通过插件数据目录里的一个小状态文件比较
各终端的最后输入时间（就是 `w` 命令显示为 IDLE 的那个 atime 信号）。
状态文件（`statusline.tty`）的内容是当前持有者的终端设备，mtime 是持有
者的心跳——会话一关，几秒内就让位。每次实时渲染还会留下一份按终端区分
的快照（`statusline.frozen.<tty>`），非活跃会话重打的就是这一行。没有
自己终端的会话（VS Code、桌面应用、无头运行）无法排序，所以不参与竞争、
始终实时渲染。门控内部的一切故障都 fail-open——坏了宁可实时，绝不冻结。
...
每个部分都能单独定制——颜色、粗体/斜体、14 套进度条字符、音量条形状、
图标，还有 `off` 隐藏任意部分。**完整目录、示例和现成搭配：
...
| `statusline.activetab` | `on` | 只在使用中的标签页更新 |
