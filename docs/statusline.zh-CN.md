...
在支持超链接的终端里，这个组件支持 **⌘+点击**：

| 目标 | ⌘+点击的动作 |
| --- | --- |
| `▶︎` / `⏸` 图标 | 切换播放/暂停 |
| 标题 — 歌手（以及 `(应用)`） | 跳到正在播放的媒体——应用带着正在播放的标签页（Safari 和支持 AppleScript 的 Chromium 系）或已定位到当前曲目（Music）来到最前；其他应用只把应用带到最前 |
| 进度条 | seek——10 个格子各自跳到对应位置（5%、15%、… 95%） |

工作原理：这些部分被包在指向本地 `claude-media://` URL scheme 的 OSC 8
超链接里。开启状态栏时会顺带生成一个小处理器应用
（`ClaudeMediaClick.app`——用 macOS 自带的 `osacompile` 生成到插件数据
目录，零第三方代码）并注册到 LaunchServices。点击后运行
`media.sh open-url`，它只接受三个无害动作——toggle、activate、按百分比
seek——其余一律拒绝。浏览器播放时，会把网页内容辅助进程解析成其所属
应用再激活（例如 `com.openai.atlas.web` → ChatGPT Atlas），并在应用
允许的范围内落到媒体本身：选中标题与曲目匹配的窗口+标签页（Safari、
Chrome、Edge、Brave、Vivaldi、Opera），或定位到当前曲目（Music）。
没有脚本接口的应用（如 ChatGPT Atlas、Spotify）则止步于来到最前。
第一次跳标签页时会为 `ClaudeMediaClick.app` 请求一次自动化授权——
拒绝后会静默地只保留应用激活。

说明：

- **终端支持**：iTerm2、Ghostty、WezTerm、Kitty、VS Code、Alacritty
  0.11+ 都能让 OSC 8 链接可点击（通常是 ⌘+点击；自定义 URL scheme
  第一次可能会询问确认）。不支持超链接的终端（如 Terminal.app）看不到
  链接字节，只会显示普通组件。tmux 3.4+ 会透传超链接。
- 点击后，组件会在下一次刷新（≤1 秒）反映变化：图标翻转、进度条跳走。
- 开关是 `statusline.links`（默认 `on`）：
  `/media:config statusline.links off` 恢复成无链接的普通组件；重新
  开启会（重新）生成并注册处理器应用，若构建失败则**拒绝开启（exit
  3）**。链接只在处理器应用真实存在时才渲染——没人应答的链接比没有
  链接更糟。`/media:doctor` 会报告处理器状态（`Click links`）。
- URL scheme 天生是系统级的——任何应用都能打开 `claude-media://` URL。
  处理器暴露的全部能力就是播放/暂停、把播放器带到前台、seek：最坏也
  只是烦人程度，和键盘上的媒体键同级。
- 卸载插件（或运行 `media.sh statusline uninstall`）时，处理器应用会和
  其余接线一起注销并删除。

## 更新跟着你正在用的标签页走

同时开着多个 Claude Code 会话时，每个会话都在刷新自己的状态栏——但
正在播放组件**只在你实际使用的那个会话里更新**。其余会话把组件的最后
一行冻结住：曲目信息还在，只是进度条和时间停下来，逐次刷新的读取工作
也不再发生（你原有的状态栏在每个会话里照常实时运行，分毫不动——被
门控的只有插件自己这一行）。无需任何设置：终端最后接收输入的那个会话
拿到实时组件——打字、滚动、切换到它的标签页都算输入（Claude Code
开启了终端 focus reporting，所以仅仅切换标签页就够了）——切回来后
一两次刷新内组件就会追上进度。

工作原理：状态栏命令在没有控制 tty 的情况下运行，所以组件沿进程祖先链
找到持有会话终端的 Claude Code 进程，再通过插件数据目录里的一个小状态
文件（`statusline.tty`——内容是当前持有者的设备，mtime 是持有者的心跳）
比较各终端的最后输入时间（`w` 显示为 IDLE 的那个 atime 信号）。会话
关闭后心跳停止，几秒内就会让出实时组件。每次实时渲染还会留下一份
按终端的快照（`statusline.frozen.<tty>`）——非活跃会话重印的正是这一行。
没有自己 tty 的会话（VS Code、桌面应用、headless 运行）无法排序，因此
不参与竞争、始终实时渲染。门控内的一切失败都是 fail-open——坏了宁可
实时渲染，绝不冻结。
...
4. **卸载插件后，一切都会自己还原。** Claude Code 没有可供插件使用的
   ...
   `settings.json`，删掉自己和备份文件，并以同样的方式注销、移除
   `claude-media://` 点击处理器应用。什么都不留——卸载后一秒之内，
   你的状态栏就恢复成原来的模样。
