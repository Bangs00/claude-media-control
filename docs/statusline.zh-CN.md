...
真正的播放信息读取每秒最多一次，时间和进度条每秒动一次也正是靠这个节拍。
...
| --- | --- |
| `▶︎` / `⏸` 图标 | 切换播放/暂停 |
| 曲名 — 歌手、`(应用)` | 跳到正在播放的媒体：正在播放的浏览器标签页（Safari、Chrome、Edge、Brave、Vivaldi、Opera、ChatGPT Atlas）或 Music 的当前曲目——其他应用只是调到最前 |
...
媒体本身：选中标题匹配曲目的窗口+标签页（后台标签页的标题还没跟上
切歌时，会回退到已知音乐播放站点的第一个标签页），或让 Music 定位到
当前曲目。（ChatGPT Atlas 是原生外壳内嵌 Chromium 引擎的结构，跳转标签页时脚本
控制的是内嵌引擎。）没有可供脚本落地的应用（如 Spotify）就止步于调到
最前。卸载
