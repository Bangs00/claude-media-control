...
运行 `/media:statusline`，对着预设（Standard / Everything / Compact）的
预览挑一个，并当场决定并成一行还是分行显示；选 `Custom…` 就能一步步挑出
要显示的条目和排在最前面的条目，拼出自己的排布。`/media:config` 里也内置
一个快捷的预设选择器:
...
- **布局**: 并成一行，或者每组各占一行（`statusline.multiline`）。`app`
  会贴在 track 那一组里；`progressbar` 和 `time` 在顺序上相邻时共用一组
  （分行布局下即共用一行）。track 和 `output` 相邻时也一样——把输出设备
  紧跟在 track 后面，分行时它就留在 track 那一行。
...
时间在前（`statusline.fields "time,progressbar,track,app"`）:
