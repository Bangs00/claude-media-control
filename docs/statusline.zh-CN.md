...
运行 `/media:statusline` 排布组件。预设（Standard / Stacked）对着预览挑。
`Custom…` 则按下面的图例组一串数字模式，直接敲进聊天输入框:
...
`track app progressbar time`；`/media:config` 里也内置一个快捷的预设
选择器。
