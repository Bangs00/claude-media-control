...
다이얼, 믹싱 콘솔. 색은 그 실물에서 끌어왔습니다. 인광체의 발광선,
안료, 문서로 남은 표준에서요. 아래 모든 명령은 실제 `media.sh config`
검증을 통과했고, 모든 GIF는 렌더러의 실제 출력(초당 1프레임)입니다.
(전부 가상의 트랙 — *Modem Chorus*의 *Rented Sunsets*, 가상의 앱
*Aux*에서 재생 중.)
...
거의 검정에 가까운 바탕 위 주황 셀 — 네온 가스 패널입니다. 셀은 켜졌거나
꺼졌거나 둘 중 하나이고, 중간이 없습니다.
...
이 주황은 네온 자신의 색입니다 — 가시광에서 가장 센 두 선이 585 nm와
640 nm에 있습니다. 바를 `rise`, `fade`, `corner`로 바꾸면 같은 채움이
셀 단위가 아니라 8분의 1, 3분의 1, 4분의 1씩 자랍니다. 이 패널의
도트 매트릭스 판을 원하면 `braille`(부분 셀 쌍은 `stipple`)을 쓰세요.
...
앰버 인광 사촌을 원하면 `#33ff33`/`#22bb33`/`#22aa22`를
`#ffb000`/`#cc8400`/`#996300`으로 바꾸세요.

## Goban

점판암의 검정과 대합 조개의 흰색 — 바둑돌입니다. 검은 돌은 작아 보이기
때문에 0.3 mm 크게 깎아, 둘이 같은 크기로 보이게 만듭니다.
...
모직 위 금사 줄 — 소매의 셰브런입니다. 1777년부터 딱 한 가지를
뜻해 왔습니다. 복무한 시간이요.
...
황동과 니스칠 참나무, 그리고 경계에서 점이 굵어져 대시가 됩니다 —
전신의 가장 오래된 규칙입니다. 대시는 점 셋을 붙여 놓은 것이니까요.
...
40칸 헤어라인 눈금과 붉은 바늘 — 실버 페이스 리시버의 백라이트 튜너
다이얼입니다. 글자는 아이스 블루로.
...
경화강과 황동 손잡이. 머리가 헤어라인 눈금을 미끄러지다 눈금과 눈금
*사이*에 섭니다 — 1631년부터 버니어가 해 온 일이 바로 그것입니다.
...
길이가 40이 아니라 36인 이유가 있습니다. `glide` 머리는 반 칸 위치에서만
...
그래서 화음이 아니라 실제 음원처럼 움직입니다. 블록 높이 판을 원하면
...
이 빨강은 최초로 눈에 보인 LED 자신의 색입니다 — 갈륨 비소 인화물,
655 nm, 1962년. `spectrum`을 `cava`로 바꾸면 같은 분석을 braille 점으로,
가로 밀도 두 배로 그립니다.

## Seiche

호수 전체가 그릇 안에서 흔들립니다 — 분지의 폭이 얼마든 그 안에 꼭 맞는
정상파입니다. 이 바가 길이에 상관없이 늘 같은 2.5개의 물결을 보여 주는
이유이기도 합니다.
...
남색에서 초록으로 가는 방향은 호수색 척도가 놓인 방향 그대로입니다 —
...
그림자를 중심에서 바깥으로 던집니다. 빛이 파동임을 증명하려고 만든
장치입니다.
...
바가 길어지면 박동이 넓어지는 게 아니라 더 많아지는 이유가 여기 있습니다.
...
braille로 그리는데, 행이 넉넉해서 스파이크뿐 아니라 작은 P·T 융기까지
보여 줍니다. `ekg`는 중심선 둘레가 아니라 바닥에서 위로 박동을 그립니다.
...
점선 길을 걸어가는 고양이, 따뜻한 종이색으로 — 터미널의 생물입니다.
