...
| 부분 | 대상 | ⌘+클릭 동작 |
| --- | --- | --- |
| `toggle` | `▶︎` / `⏸` 아이콘 | 재생/일시정지 토글 |
| `track` | 제목 — 가수 | 재생 중인 미디어로 이동: 재생 중인 브라우저 탭(Safari, Chrome, Edge, Brave, Vivaldi, Opera) 또는 Music의 현재 트랙 — 그 외 앱은 앱만 앞으로 |
| `app` | `(앱)` | 앱 이름을 눌러도 같은 곳으로 |
| `seek` | 진행 바 | seek — 모든 칸이 각자의 위치로 점프 (기본 20칸이면 2.5%, 7.5%, … 97.5%; 바가 길수록 더 촘촘하게) |
...
- 클릭 결과는 다음 틱(1초 이내)에 반영됩니다: 아이콘이 바뀌고 바가
  점프합니다.
- 켜고 끄기: `/media:config statusline.links off`면 링크 없는 일반
  세그먼트로 돌아가고, `on`이면 네 부분 모두 다시 ⌘+클릭에 반응합니다.
- **원하는 것만** 골라도 됩니다. 목록에서 빠진 부분은 링크 없이 그대로
  표시됩니다:

  ```bash
  /media:config statusline.links toggle,seek     # 아이콘과 바만 클릭 가능, 글자는 그대로
  /media:config statusline.links track,app       # 글자만 클릭 가능, 아이콘과 바는 그대로
  ```

  하나라도 켜져 있으면 핸들러 앱이 있어야 하니, 링크를 켜거나 목록을
  지정할 때마다 다시 만듭니다. 빌드가 실패하면 거부됩니다(exit 3) —
  아무도 받지 않는 링크는 없느니만 못하니까요.
...
선택하거나(Safari에서는 백그라운드 탭의 제목이 곡 전환을 아직 따라오지
못했다면 알려진 음악 플레이어 사이트의 첫 탭으로 폴백합니다) Music의
현재 트랙을 표시합니다. 그 외 앱(예: ChatGPT Atlas, Spotify)은 앞으로
나오는 데서 멈춥니다. 플러그인을
...
모든 부분을 하나하나 따로 꾸밀 수 있습니다 — 색(이름 또는 `#ff8800` 같은
헥스 코드), 굵게/기울임, 36가지 진행
바 문자, 바 길이(1–60칸, 기본 20), 볼륨 바 모양, 아이콘, 그리고 `off`로
숨기기까지. **전체 카탈로그와 예시:
[docs/styles.ko.md](styles.ko.md), 완성 레시피:
...
| `statusline.links` | `on` | ⌘+클릭 가능한 부분: `toggle`, `track`, `app`, `seek` (`on` = 전체, `off` = 없음) |
