...
| --- | --- |
| `▶︎` / `⏸` 아이콘 | 재생/일시정지 토글 |
| 제목 — 가수, `(앱)` | 재생 중인 미디어로 이동: 재생 중인 브라우저 탭(Safari, Chrome, Edge, Brave, Vivaldi, Opera, ChatGPT Atlas) 또는 Music의 현재 트랙 — 그 외 앱은 앱만 앞으로 |
...
선택하거나(백그라운드 탭의 제목이 곡 전환을 아직 따라오지 못했다면
알려진 음악 플레이어 사이트의 첫 탭으로 폴백합니다) Music의 현재
트랙을 표시합니다. (ChatGPT Atlas는 네이티브
셸이 Chromium 엔진을 품은 구조라, 탭 이동은 내장 엔진을 스크립팅해서
이뤄집니다.) 스크립팅으로 내려앉을 곳이 없는 앱(예: Spotify)은 앞으로
나오는 데서 멈춥니다. 플러그인을
