...
原型——荧光体终端、磁带卡座、调谐度盘、调音台。下面每条命令都通过了真实的
...
只留标题和播放位置——连 marquee 也关掉，唯一会动的只有时间。
...
40 格细线刻度上一根红针——银面收音机的背光调谐度盘，冰蓝色字。
