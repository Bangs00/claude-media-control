...
播放时缓缓漂移，暂停即静止。`wave` `pulse` `eq` 像下文的可视化器一样
铺满**整条**，用颜色标出进度；`notes` 填充到边界为止。
按长度 20 和 10 满条展示：
...
每一种都有一个密度翻倍的**点阵孪生版**：`swell`（wave）、`bars`（eq）、
`ekg`（pulse）——见下文[音频可视化](#音频可视化)。

### 音频可视化

七种预设把进度条变成音乐可视化器。**块**版用八分块的高度（`▁`..`█`）；
...
七种预设全部铺满**整条**，而不只是已播放的部分——上面的 `wave` `pulse`
`eq` 也一样。开着颜色时，强调色与暗淡的分界标出进度；关掉颜色时，
未播放的尾段会压低高度，所以光看形状也能读出进度。
