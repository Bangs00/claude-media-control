...
`/media:statusline` を実行すると、プリセット（Standard / Everything /
Compact）をプレビューで見比べながら選べるほか、1 行にまとめるか行を分けるか
もその場で決められます。`Custom…` を選ぶと、表示する項目と先頭に置く項目を
ひとつずつ選んで自分だけの配置を作れます。簡易版のプリセットピッカーは
...
- **レイアウト**: 1 行にまとめるか、グループごとに行を分けるか
  ...
  （複数行レイアウトでは 1 行）になります。track と `output` も同様で、
  出力デバイスを track のすぐ後ろに置けば、行を分けても track と同じ行に
  残ります。
