...
| --- | --- |
| `▶︎` / `⏸` アイコン | 再生 / 一時停止の切り替え |
| 曲名 — アーティスト、`(アプリ)` | 再生中のメディアへジャンプ: 再生中のブラウザタブ（Safari、Chrome、Edge、Brave、Vivaldi、Opera、ChatGPT Atlas）、または Music の現在のトラック — それ以外のアプリは前面に出るだけ |
| プログレスバー | シーク — どのマスもそれぞれの位置へジャンプ（デフォルトの 20 マスなら 2.5%、7.5%、… 97.5%。バーが長いほど刻みは細かく） |
...
一致するウィンドウ+タブを選択するか（バックグラウンドタブのタイトルが
曲の切り替わりにまだ追いついていない場合は、既知の音楽プレイヤー
サイトの最初のタブへフォールバック）、Music の現在のトラックを表示します。
（ChatGPT Atlas はネイティブのシェルが Chromium エンジンを内蔵する構造
のため、タブへのジャンプは内蔵エンジンへのスクリプトで行われます。）
スクリプトで着地できる先のないアプリ（例: Spotify）は前面に出るところ
までです。プラグインをアンインストールすると（または
