...
| 部分 | 対象 | ⌘+クリックの動作 |
| --- | --- | --- |
| `toggle` | `▶︎` / `⏸` アイコン | 再生 / 一時停止の切り替え |
| `track` | 曲名 — アーティスト | 再生中のメディアへジャンプ: 再生中のブラウザタブ（Safari、Chrome、Edge、Brave、Vivaldi、Opera）、または Music の現在のトラック — それ以外のアプリは前面に出るだけ |
| `app` | `(アプリ)` | アプリ名からも同じところへ |
| `seek` | プログレスバー | シーク — どのマスもそれぞれの位置へジャンプ（デフォルトの 20 マスなら 2.5%、7.5%、… 97.5%。バーが長いほど刻みは細かく） |
...
- スイッチ: `/media:config statusline.links off` でリンクなしの通常
  セグメントに戻り、`on` ならどの部分も再びクリックできます。
- **クリックしたい部分だけ**を指定することもできます。指定しなかった部分は
  リンクなしで表示されます:

  ```bash
  /media:config statusline.links toggle,seek     # アイコンとバーだけクリック可能、文字はそのまま
  /media:config statusline.links track,app       # 文字だけクリック可能、アイコンとバーはそのまま
  ```

  どれか 1 つでもオンならハンドラーアプリが必要になるため、リンクを
  オンにしたり、リストを指定したりするたびにハンドラーアプリを作り直し
  ます。そのビルドが失敗すると拒否されます（exit 3）— 誰も応えないリンク
  は、ないほうがましだからです。
...
クリック可能な部分は、ローカルの `claude-media-control://` URL スキームを
指す OSC 8 ハイパーリンクです。ステータスラインを有効にすると、小さな
...
一致するウィンドウ+タブを選択するか（Safari では、バックグラウンドタブ
のタイトルが曲の切り替わりにまだ追いついていない場合、既知の音楽
プレイヤーサイトの最初のタブへフォールバック）、Music の現在のトラック
を表示します。それ以外のアプリ（例: ChatGPT Atlas、Spotify）は前面に
出るところまでです。プラグインをアンインストールすると（または
...
アクセント、**太字**の曲名と経過時間、*斜体*のアーティスト、その他は
控えめに薄く — 標準 16 色の SGR だけを使うので、実際の色味は
ターミナルのパレットが決めます。

すべてのパーツは個別にスタイリングできます — 色（色名または `#ff8800` の
ような hex コード）、太字 / 斜体、36 種類の
...
| `statusline.links` | `on` | ⌘+クリック可能な部分: `toggle`、`track`、`app`、`seek`（`on` = すべて、`off` = なし） |
