...
| `━━━━━━────` | `style.progressbar.style`（文字） | `line` |
...
文字は `style.progressbar.style` で決まります。ステータスラインのバーは
10 マス、`/media:now` の返信に出る 20 マスのバーも同じ文字で描かれるので、
2 つの表示は常に一致します。文字の選択は色をオフにしていても有効です。

![バーのプリセットと音量の形が毎秒 1 フレームで動く様子](styles.gif)

### 静的なプリセット

60% 時点の見た目です（`smooth` のみ、部分ブロックが見える 56%）:
