...
`spectrum` `mirror` `cava` `ripple` は再生済みの部分だけでなく**バー全体**に
広がります。色がオンなら、アクセントと dim の分かれ目が進み具合を示します。
色がオフなら、未再生の末尾は高さを抑えて描かれるので、形だけでも進み具合が
読み取れます。`swell` `bars` `ekg` は上の波形と同じく境界まで塗られます。
