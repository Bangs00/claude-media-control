...
`wave` `pulse` `eq` は下記のビジュアライザーと同じく**バー全体**に広がり、
色で進み具合を示します。`notes` は境界まで塗られます。
長さ 20 と 10 での全体表示:
...
それぞれに密度が 2 倍の**点字の双子**があります: `swell`（wave）、`bars`（eq）、
`ekg`（pulse）— 下記の[オーディオビジュアライザー](#オーディオビジュアライザー)を参照。

### オーディオビジュアライザー

7 つのプリセットがバーを音楽ビジュアライザーに変えます。**ブロック**系は
...
7 つすべてが再生済みの部分だけでなく**バー全体**に広がり、上の
`wave` `pulse` `eq` も同じです。色がオンなら、アクセントと dim の
分かれ目が進み具合を示します。色がオフなら、未再生の末尾は高さを抑えて
描かれるので、形だけでも進み具合が読み取れます。
