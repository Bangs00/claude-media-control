...
会把各种布局**以预览的形式**摆在你面前挑，显示哪些内容（曲目、应用、
进度条、时间、输出设备）、按什么顺序、放在第几行，全都由你定。预设
（Standard / Stacked）不合心意的话，选 `Custom…` 打一串数字模式就行，
比如 `12/34/5` —— 数字代表条目，`/` 表示换行，数字顺序就是显示顺序。
条目严格按保存的顺序渲染，所以"时间放最前面"、"输出设备提到前面"这类要求
也能原样实现。超过 30 格的长标题会以跑马灯方式滚动
