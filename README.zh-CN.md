...
- **点击即可操控**——在支持超链接的终端（iTerm2、Ghostty、WezTerm、Kitty、
  ...
  [docs/statusline.zh-CN.md](docs/statusline.zh-CN.md)）。
- **更新只在你正在用的标签页**——同时开着多个 Claude Code 会话时，组件
  只在你实际使用的标签页里更新（打字、滚动、切换过去都算）；其他标签页
  保留它的最后一行、冻结不动——只是进度条和时间停了——你一回来立刻
  追上。你原有的状态栏在每个会话照常实时运行——被门控的只有插件这一行。
  想每个会话都更新：`/media:config statusline.activetab off`。
- **定制全在 `/media:statusline`**——外观的一切都在这一个中枢里。开关条目，
  挑布局或直接敲一串 `123/456` 这样的数字模式（数字代表条目——曲目、
  应用、音量、进度条、时间、输出设备——`/` 表示换行），还能逐项定制
  样式：加粗/斜体/颜色、播放/暂停强调色、进度条字符（从默认 `line`
  `━━──`、`smooth` 部分块、`knob` 滑块圆点到播放时滚动的
  `wave`/`pulse`/`eq`/`notes`，共 14 种预设，或任意两个字符）、
  音量图标和柱条形状
