...
| "调整状态栏布局" | `/media:statusline` | 对着预览挑布局，或用数字模式自定义分行和顺序 |
| "把曲名改成青色" | `/media:style` | 逐项定制状态栏样式——每个部分的加粗/斜体/颜色、进度条字符、音量图标 |
| "配置状态栏" | `/media:config` | 交互式设置——布局，外加每一个显示开关（进度条、历史、颜色、跑马灯） |
| — | `/media:doctor` | 诊断构建 / 权限 / 回退路径 |

还可以把正在播放的歌放进状态栏——见
[docs/statusline.zh-CN.md](docs/statusline.zh-CN.md)。`/media:statusline`
会把各种布局**以预览的形式**摆在你面前挑，显示哪些内容（曲目、应用、音量、
进度条、时间、输出设备）、按什么顺序、放在第几行，全都由你定。预设
（Standard / Stacked）不合心意的话，选 `Custom…` 直接在聊天输入框打一串
数字模式就行，比如 `123/456` —— 数字代表条目，`/` 表示换行，数字顺序就是
显示顺序。条目严格按保存的顺序渲染，所以"时间放最前面"、"输出设备提到前面"
这类要求也能原样实现。音量条目显示为图标 + 随音量升降的柱条 + 百分比
（`🔉 ▄ 45%`），输出设备的图标跟着设备类型走（`🎧` 蓝牙、`📺` HDMI、`📶`
AirPlay、`🔊` 扬声器）。超过 30 格的长标题会以跑马灯方式滚动
（`statusline.marquee`），状态栏组件自带 ANSI 样式——图标和进度条的颜色跟随
播放状态、曲名和已播放时间加粗、歌手斜体。想要纯文本的话，运行
`/media:config statusline.color off`（也支持 `NO_COLOR` 环境变量）。每个
部分还能用 `/media:style` 单独定制——曲名、歌手、应用、时间、音量条和
百分比、输出设备的加粗/斜体/颜色，进度条的播放/暂停颜色和字符（`wave`
`~~~~--`、`line`、`dots`，或任意两个字符），以及音量图标（`♪`、隐藏，或
默认的分级图标）。每个键都可以用 `reset` 单独恢复默认值，`/media:config
style reset` 则一键全部还原。
...
在 `/media:config` 里勾选"输出设备条目"，或者用 `/media:statusline` 把它
放到任意位置。
