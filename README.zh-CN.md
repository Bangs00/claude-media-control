...
- **排布用 `/media:statusline`** —— 对着预览挑预设，或直接敲一串
  `123/456` 这样的数字模式。数字代表条目（曲目、应用、音量、进度条、
  时间、输出设备），`/` 表示换行，数字顺序就是显示顺序。
- **样式用 `/media:style`** —— 每个部分的加粗/斜体/颜色、播放/暂停强调色、
  进度条字符（默认 `line` `━━──`；还有 `blocks` `██░░`、`wave` `~~--`、
  `dots` `●●○○`，或任意两个字符）、以及音量图标。每个键都能用 `reset`
  单独恢复默认，也可以一键全部还原。
