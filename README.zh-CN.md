...
| — | `/media:config` | 开关各项显示功能（进度条、状态栏、频谱） |
| — | `/media:doctor` | 诊断构建 / 权限 / 回退路径 |

还可以把正在播放的歌放进状态栏——见
[docs/statusline.zh-CN.md](docs/statusline.zh-CN.md)。显示哪些内容（曲目、
进度条、时间、频谱）、是并成一行还是分行，都在 `/media:statusline` 里选。
状态栏组件自带 ANSI 样式——图标和进度条的颜色跟随播放状态、曲名加粗、歌手
斜体、频谱着色（纯色，或用 `spectrum.style` 开启按位置循环的彩虹色）。想要
纯文本的话，运行 `/media:config statusline.color off`（也支持 `NO_COLOR`
环境变量）。
