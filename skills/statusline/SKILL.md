...
filling it — the creature stands where the track has got to, so its *position*
is the progress, and its legs cycle once a second while playing (they hold
still when paused): `cat` `━━━ᓚᘏᗢ┈┈┈┈` · `snake` `━━━ᔓᔕᔓ╌╌╌╌` ·
`duck` `≈≈≈ᕬᗢ~~~~~` · `bird` `━━━⌒v⌒····` — each on its own kind of track.
`sprite` is the bring-your-own one: `style.progressbar.sprite` takes the
frames it cycles (`"ᓚᘏᗢ ᓚᘐᗢ"`, or a single glyph such as `"🚀"` to walk
...
- **Progress bar & time** — 4 questions:
  ...
  - "Bar characters" (`style.progressbar.style`): Keep current /
    ...
    two glyphs like `#-`
  - Only when the chosen style is `sprite`, follow up with "Sprite frames"
    (`style.progressbar.sprite`): Keep current / `Blinking dot ● ○ (default)` /
    `Walking cat ᓚᘏᗢ ᓚᘐᗢ` / Other → space-separated frames cycling one per
    second, or a single glyph like `🚀` to walk without animating. The two
