...
- `statusline.links` — cmd+click actions in the statusline segment (default `on`): the ▶︎/⏸ icon toggles playback, title/artist jump to the playing app, each progress-bar cell seeks. Enabling builds + registers the local `claude-media://` handler app and is refused (exit 3) when that build fails; disabling renders the segment plain. Works in hyperlink-capable terminals (iTerm2, Ghostty, WezTerm, Kitty, VS Code)
