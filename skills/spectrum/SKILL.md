...
- Present the spectrum line(s) verbatim inside a fenced code block so the block glyphs stay aligned — they are already formatted. `--live <seconds>` streams several frames over that period (Claude Code chat is static text, so this is a scroll, not a smooth animation).
...
Coloring: the bars are tinted per `spectrum.style` (`solid`, default, in `spectrum.color` — default cyan) or `rainbow` (front-to-back color cycle by bar position, amplitude-agnostic). The tint shows in the statusline segment and when running `media.sh spectrum` directly in a terminal; lines relayed into this chat are plain glyphs (static text). Change it with `/media:config spectrum.style rainbow` or `/media:config spectrum.color <color>`.
